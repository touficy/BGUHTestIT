...
function getfaq() {
    ...
    $.ajax({
        type: 'GET',
        url: "https://host.optimalsolutionslebanon.com/~bguh/api/listFaq",
     ...
     data:{
        ...
        for (var i = 0; i < json['response']['faqs'].length; i++) {
            if (localStorage.getItem("lang") == 'ar') {
                var acc = '<li class="accordion-item"><a href="#" class="item-content item-link">' +
                    '<div class="item-inner">' +
                    '<div class="item-title" id= "' + json['response']['faqs'][i]["id"] + '" >' + json['response']['faqs'][i]["question"] + '</div>' +
                    '</div></a>' +
                    '<div class="accordion-item-content">' +
                    '<div class="content-block">' +
                    '<p>' + json['response']['faqs'][i]["answer"] + '</p>' +
                    '</div>' +
                    '</div>' +
                    '</li>';
                $("#acc").append(acc);
